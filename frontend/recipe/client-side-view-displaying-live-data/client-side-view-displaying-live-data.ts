...
declare global {
  interface HTMLElementTagNameMap {
    'client-side-view-displaying-live-data': LiveDataView;
  }
}
...
declare module 'react' {
  namespace JSX {
    interface IntrinsicElements {
      'client-side-view-displaying-live-data': DetailedHTMLProps<HTMLAttributes<LiveDataView>, LiveDataView>;
    }
  }
}
...
export class LiveDataView extends Recipe {
  ...
  render() {
    return html`
      ...
      <p>
        ...
        <a href="/show-real-time-updating-data">See this example using the server-side Java API.</a>
      </p>

      <vaadin-chart>
        <vaadin-chart-series title=${this.#ticker} .values=${this.#priceHistory}></vaadin-chart-series>
      </vaadin-chart>
    `;
  }
  ...
  connectedCallback() {
    ...
    const priceSource = new EventSource(`${window.location.origin}/stock-price`);
    ...
    priceSource.addEventListener('message', (e) => {
      ...
      this.#priceHistory = [...this.#priceHistory, [this.#priceHistory.length, parseFloat(this.#currentPrice)]];
    });
