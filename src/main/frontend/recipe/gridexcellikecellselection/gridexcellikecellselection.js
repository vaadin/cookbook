...
window.Vaadin.Flow.custom.initGridExcelCellSelection = function (grid) {
  ...
  function mouseMoveEventListener(event) {
    if (!startTdOfMove) {
      ...
      if (td) {
        ...
        let initRow =
          startRowOfMove <= currentRow ? startRowOfMove : currentRow;
        let maxRow = startRowOfMove <= currentRow ? currentRow : startRowOfMove;

        let initCell =
          startCellOfMove <= currentCell ? startCellOfMove : currentCell;
        let maxCell =
          startCellOfMove <= currentCell ? currentCell : startCellOfMove;
  ...
  getInnerTable().addEventListener("scroll", (event) => {
    ...
    table.querySelectorAll("td").forEach((td) => {
      if (
        selectedIndexes.has(td.parentNode.index) &&
        selectedColumns.has(td._column._flowId)
      ) {
        addSelectedStyle(td);
      } else {
  ...
  grid.addEventListener("mouseup", (e) => {
    startTdOfMove = undefined;
    grid.dispatchEvent(
      new CustomEvent("excel-like-selected-cells", {
        detail: {
