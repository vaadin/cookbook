...
public class HighlightRouterLinkWithDefaultRoute extends Recipe implements RouterLayout {
    ...
    private HighlightCondition<RouterLink> buildHighLightConditionFor(Class<?> routerTargetClass) {
        return (link, afterNavigationEvent) -> {
            return afterNavigationEvent.getActiveChain()
                    .stream().map(HasElement::getClass)
                    .anyMatch(targetClass -> targetClass.isAssignableFrom(routerTargetClass));
        };
    }
