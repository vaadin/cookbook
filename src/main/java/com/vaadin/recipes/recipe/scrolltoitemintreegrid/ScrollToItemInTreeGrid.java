...
@Metadata(howdoI = "Scroll a TreeGrid to an item and expand nodes", description = "Code example for scrolling to a specific item in a TreeGrid, expanding nodes as needed.", tags = {
    Tag.GRID })
public class ScrollToItemInTreeGrid extends Recipe {
  ...
  public ScrollToItemInTreeGrid() {
    ...
    buttons.add(new Button("Expand and scroll to first", e -> {
      grid.expand(grandDad, dad);
      grid.scrollToItem(firstChild);
    }));
    buttons.add(new Button("Expand and scroll to last", e -> {
      grid.expand(grandDad, dad);
